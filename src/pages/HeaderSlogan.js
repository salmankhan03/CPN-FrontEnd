...
import HeaderSloganTable from "components/HeaderSlogan/HeaderSloganTable";

const HeaderSlogan = () => {
  const { toggleDrawer, lang } = useContext(SidebarContext);
  const { data , loading } = useAsync(HeaderSloganServices.getSlogan);
  ...
  return (
    <>
      <PageTitle>{"Header Slogan"}</PageTitle>
      ...
      <Card className="min-w-0 shadow-xs overflow-hidden bg-white dark:bg-gray-800 mb-5">
        <CardBody>
          <form onSubmit={handleSubmitSlogan} className="py-3 grid gap-4 lg:gap-6 xl:gap-6  xl:flex">
          ...
          >
            <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
              <Input
                ...
                placeholder={"Search By Slogan"}
                onChange={handleSubmitSlogan}
              />
            </div>
          </form>
        </CardBody>
      ...
      {loading ? (
          ...
          <Table>
            <TableHeader>
              <tr>
                ...
                <TableCell>{"Slogan Url"}</TableCell>
            ...
            <HeaderSloganTable lang={lang} isCheck={isCheck} slogans={data1} setIsCheck={setIsCheck} />
          </Table>
          <TableFooter>
            <Pagination
        ...
        <NotFound title="Sorry, There are no Slogan right now." />
      )}
    </>
  );
};
